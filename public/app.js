...
const TOUR_NAMES={sea:"Sea to Sky Gondola",grouse:"Grouse Mountain",whistler:"Whistler Village"};
const SLOTS=["Morning — 9:00 AM","Noon — 1:00 PM","Evening — 5:00 PM"];
let settings={prices:{sea:125,grouse:100,whistler:125,addon:100},images:{}};
let adminPassword="",currentTour="sea",selectedSlot="",bookedSlots=[];
const el=id=>document.getElementById(id);
async function api(url,opts={}){const res=await fetch(url,{headers:{"Content-Type":"application/json","x-admin-password":adminPassword},...opts});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||"Error");return data}
function todayISO(){const d=new Date();d.setMinutes(d.getMinutes()-d.getTimezoneOffset());return d.toISOString().split("T")[0]}
function bg(url){return url?`url('${url}')`:""}
async function loadSettings(){settings=await api("/api/settings");applySettings()}
function applySettings(){const p=settings.prices||{};cardSea.textContent=p.sea;cardGrouse.textContent=p.grouse;cardWhistler.textContent=p.whistler;cardAddon.textContent=p.addon;addonText.textContent=p.addon;adminSea.value=p.sea;adminGrouse.value=p.grouse;adminWhistler.value=p.whistler;adminAddon.value=p.addon;const i=settings.images||{};hero.style.setProperty("--hero",bg(i.hero));imgSea.style.backgroundImage=bg(i.sea);imgGrouse.style.backgroundImage=bg(i.grouse);imgWhistler.style.backgroundImage=bg(i.whistler)}
async function openBooking(tour){currentTour=tour;selectedSlot="";modalTitle.textContent="Book "+TOUR_NAMES[tour];modalSubtitle.textContent="$"+settings.prices[tour]+" / person";addonWrap.style.display=tour==="whistler"?"block":"none";addonCheck.checked=false;tourDate.min=todayISO();ticketBox.style.display="none";bookingModal.classList.add("active");await loadAvailability();calculate()}
...
async function adminLogin(){
  ...
  try { await renderOrders(); } catch(e) { console.log(e); }
}
async function savePrices(){const prices={sea:Number(adminSea.value),grouse:Number(adminGrouse.value),whistler:Number(adminWhistler.value),addon:Number(adminAddon.value)};await api("/api/settings",{method:"POST",body:JSON.stringify({prices})});await loadSettings();alert("Prices saved.")}
function fileToBase64(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result).split(",")[1]);r.onerror=reject;r.readAsDataURL(file)})}
async function uploadOne(input,key,images){const file=input.files[0];if(!file)return;const base64=await fileToBase64(file);const data=await api("/api/upload-image",{method:"POST",body:JSON.stringify({filename:file.name,contentType:file.type,base64})});images[key]=data.url}
